...
public class SuggestRSOScreen extends Screen {
    ...
    private static final Text HEADER = new TranslatableText("sodium-device-check.suggestRSO.header").formatted(Formatting.BOLD);
    private static final Text MESSAGE = new TranslatableText("sodium-device-check.suggestRSO.message");
    ...
    protected void init() {
        ...
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 155, 130 + i, 150, 20, new LiteralText("CurseForge"), buttonWidget -> Util.getOperatingSystem().open("https://www.curseforge.com/minecraft/mc-mods/sodium-device-check")));
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 155 + 160, 130 + i, 150, 20, new LiteralText("Modrinth"), buttonWidget -> Util.getOperatingSystem().open("https://modrinth.com/mod/sodium-device-check")));
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 155, 100 + i, 150, 20, ScreenTexts.PROCEED, buttonWidget -> {
            if (this.checkbox.isChecked()) {
                LSDCClientMod.options().notificationSettings.hideRSORecommendation = true;
                LSDCClientMod.options().writeChanges();
            }
